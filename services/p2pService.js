...
async initiateTrade(buyerId, merchantAd, data, ip = null) {
  ...
  if (fiatAmount < merchantAd.minLimit) throw new TradeError("Amount below minimum trade limit");
  if (fiatAmount > merchantAd.maxLimit) throw new TradeError("Amount exceeds maximum trade limit");
...
// 3. Set the platform fee
  ...
  try {
    ...
    if (!adUpdateResult) throw new TradeError("Insufficient liquidity or merchant ad not found");
    ...
    if (shouldPreEscrow) {
      ...
      const sourceWalletId = await resolveUserWalletId(escrowSourceUserId, currencyTarget);
      ...
      if (!transferResult) throw new TradeError("Pre-escrow transfer failed at provider");
   ...
   // 🏦 2. ATTACH THE SNAPSHOT TO THE TRADE CREATION
    ...
    setImmediate(() => {
      notifyMerchantOfTrade(trade._id).catch(err => {
        logger.error("Merchant trade notification failed", { tradeId: trade._id, error: err.stack || err.message });
      });
    });
...
if (!validStatuses.includes(trade.status)) {
  throw new TradeError(`Cannot confirm payment in status: ${trade.status}`, 409);
}
...
//   try {
//     const settledTrade = await session.withTransaction(async () => {
//       // ===============================
//       // 2️⃣ Lock trade (atomic)
//       // ===============================
//       const trade = await P2PTrade.findOneAndUpdate(
//         {
//           reference: normalizedReference,
//           status: ALLOWED_STATES.PAYMENT_CONFIRMED_BY_BUYER,
//         },
//         { $set: { status: ALLOWED_STATES.RELEASING } },
//         { session, new: true }
//       );

//       if (!trade) {
//         throw new TradeError(
//           "Trade not found or already settled",
//           409
//         );
//       }
...
//       // ===============================
//       // 4️⃣ Load required wallets
//       // ===============================
//       const sellerSpotWallet = await Wallet.findOne({
//         user_id: sellerId,
//         currency: trade.currencyTarget,
//         walletType: "USER",
//         provider: "BLOCKRADAR",
...
//       if (!sellerSpotWallet) {
//         throw new TradeError("Seller spot wallet not found", 404);
//       }
...
//       // 5️⃣ Amounts
//       // ===============================
//       const netAmount = trade.netCryptoAmount;
//       const platformFee = trade.platformFeeCrypto || 0;

//       // ===============================
//       // 6️⃣ Ledger: Release crypto
//       // ===============================
//       await Transaction.create(
//         [
//           {
//             walletId: sellerSpotWallet._id,
//             userId: sellerId,
//             type: "P2P_RELEASE",
//             amount: netAmount,
//             currency: trade.currencyTarget,
...
//               status: "COMPLETED",
//               reference: trade.reference,
//               idempotencyKey: `P2P:${trade._id}:FEE_PLATFORM`,
//               metadata: { tradeId: trade._id },
//             },
//           ],
//           { session }
...
//     // 9️⃣ Cache cleanup
//     // ===============================
//     await redisClient.del(`balances:${settledTrade.userId}`);
//     await redisClient.del(`balances:${settledTrade.merchantId}`);

//     return settledTrade.toObject();
//   } catch (error) {
//     // Idempotent retry protection
//     if (error.code === 11000) {
//       return await P2PTrade.findOne({
//         reference: normalizedReference,
//       }).lean();
//     }
//     throw error;
      ...
      const trade = await P2PTrade.findOne({
        ...
        status: {
          $in: [
            ...
            ALLOWED_STATES.COMPLETED, // allow idempotent retries
          ],
        },
      }).session(session);

      if (!trade) {
        return; // trade doesn't exist
      }
...
async autoOpenDisputesForSilentBuyers() {
    ...
    for (const trade of stuckTrades) {
        await updateTradeStatusAndLogSafe(
            ...
            {
                message: "Buyer did not confirm payment in time. Dispute opened automatically.",
                role: "system"
            },
            ALLOWED_STATES.MERCHANT_PAID
        );
