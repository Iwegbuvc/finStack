...
const submitCryptoWithdrawal = async (req, res) => {
    ...
    try {
        ...
        const wallet = await Wallet.findOne({ currency: walletCurrency, user_id: userId }).session(session);
        ...
        if (new Decimal(wallet.balance).lt(totalDeduction)) {
            throw new Error(`Insufficient balance. Need ${totalDeduction.toString()}`);
        }
        ...
        let newTransaction = await Transaction.create([{
            ...
            feeDetails: { platformFee: Number(feeAmt.toString()), isDeductedFromAmount: false },
            metadata: { destination: externalCryptoAddress, provider: 'BLOCKRADER_CRYPTO' }
        }], { session });
        ...
        try {
            ...
            return res.status(200).json({ success: true, message: "Initiated successfully" });
            ...
            throw new Error("Provider service unavailable. Funds have been returned to your wallet.");
        }
